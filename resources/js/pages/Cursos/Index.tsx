...
import { AlertTriangle, BookOpen, Layers, Pencil, Plus, Search, Trash2, Users2 } from 'lucide-react';
...
export default function AdminCursosIndex() {
    ...
    const modalidadBadge = (modalidad?: string) => {
        ...
        return <Badge className={`text-xs ${colors[modalidad ?? 'Presencial']}`}>{modalidad}</Badge>;
    };
    ...
    const filteredCursos = useMemo(() => {
        ...
        if (search.trim().length > 0) {
            ...
            data = data.filter(
                (c) => c.nombre.toLowerCase().includes(term) || c.modalidad.toLowerCase().includes(term) || String(c.arancel_base).includes(term),
            );
        }
            ...
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col gap-6 p-4">
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">Administración completa de los cursos activos e inactivos.</p>
                    </div>

                    <Button asChild className="bg-primary text-primary-foreground shadow hover:bg-primary/90">
                ...
                <Card>
                    ...
                    <CardContent>
                        <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Ej: programación, presencial, 15000…" />
                ...
                <Card className="overflow-x-auto border border-border/40 shadow-sm">
                    ...
                    <CardContent>
                        {filteredCursos.length === 0 ? (
                            <p className="py-4 text-center text-sm text-muted-foreground">No hay cursos que coincidan con la búsqueda.</p>
                        ) : (
                                    ...
                                    {filteredCursos.map((curso, i) => (
                                        ...
                                        >
                                            ...
                                            <td className="px-4 py-2 font-medium">
                                                <Link href={route('cursos.show', curso.id)} className="text-primary hover:underline">
                                                    {curso.nombre}
                                                </Link>
                                            </td>

                                            <td className="px-4 py-2">{modalidadBadge(curso.modalidad)}</td>
                                            ...
                                            <td className="px-4 py-2">
                                                {curso.arancel_base ? `$${curso.arancel_base.toLocaleString('es-AR')}` : '—'}
                                            </td>
                                            ...
                                            <td className="px-4 py-2">{estadoBadge(curso.activo)}</td>
                                                        ...
                                                        <AlertDialog>
                                                            ...
                                                            <AlertDialogContent>
                                                                <AlertDialogHeader>
                                                                    ...
                                                                    <AlertDialogDescription>
                                                                        ¿Seguro que deseas eliminar el curso <strong>{curso.nombre}</strong>? Esta
