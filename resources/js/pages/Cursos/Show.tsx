...
import { Curso, CursoHorario, pageProps } from '@/types';
import { Head, Link, usePage } from '@inertiajs/react';
import { motion } from 'framer-motion';
import { ArrowLeft, BookOpen, CalendarDays, Clock, DollarSign, UserCog, Users } from 'lucide-react';
...
export default function Show() {
    ...
    const getEstadoInscripcionBadge = (estado: string) => {
        ...
        return <Badge className={`text-xs ${variants[estado] || 'bg-gray-100 text-gray-600'}`}>{estado}</Badge>;
    };
                    ...
                    <CardContent className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <p className="flex items-center gap-1">
                                ...
                                {data.arancel_base ? data.arancel_base.toLocaleString('es-AR', { style: 'currency', currency: 'ARS' }) : '—'}
                            </p>
                        </div>
                        <div>
                            <p className="text-sm text-muted-foreground">Estado</p>
                            <Badge variant={data.activo ? 'default' : 'secondary'}>{data.activo ? 'Activo' : 'Inactivo'}</Badge>
                        </div>
                    </CardContent>
                </Card>
                                ...
                                <tbody>
                                    {data.horarios.map((h, i) => (
                                        ...
                                        >
                                            ...
                                            <td className="px-4 py-2">{h.hora_inicio ? formatHoraLocal(h.hora_inicio) : '—'}</td>
                                            <td className="px-4 py-2">{h.hora_inicio && h.duracion_min ? horaFin(h) : '—'}</td>
                                ...
                                <tbody>
                                    {data.inscripciones.map((ins, i) => (
                                        ...
                                        >
                                            ...
                                            <td className="px-4 py-2">{ins.fecha_inscripcion ? formatFechaLocal(ins.fecha_inscripcion) : '—'}</td>
