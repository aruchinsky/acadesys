...
import { ArrowLeft, BookOpen, CalendarDays, Clock, PlusCircle, Trash2, UserCog } from 'lucide-react';

export default function Create({ profesores }: pageProps & { profesores: User[] }) {
    const { data, setData, post, processing, errors } = useForm({
        ...
        horarios: [] as { dia_en_texto: string; hora_inicio: string; duracion_min: string; sala: string; turno: string }[],
    });

    const addHorario = () => {
        setData('horarios', [...data.horarios, { dia_en_texto: '', hora_inicio: '', duracion_min: '', sala: '', turno: '' }]);
    };
    ...
    const duplicateHorario = (index: number) => {
        ...
        setData('horarios', [...data.horarios, { ...item }]);
    };
            ...
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col gap-6 p-4">
                ...
                <div className="flex items-center justify-between">
                    <h1 className="flex items-center gap-2 text-2xl font-semibold text-foreground">
                        <BookOpen className="h-6 w-6 text-primary" /> Nuevo Curso
                    </h1>
                    ...
                    <Card>
                        ...
                        <CardContent className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            ...
                            <div>
                                ...
                                <Select value={data.modalidad} onValueChange={(v) => setData('modalidad', v as 'Presencial' | 'Virtual')}>
                                    <SelectTrigger>
                        ...
                        <CardContent>
                            {data.horarios.map((h, i) => (
                                <motion.div
                                    ...
                                    animate="visible"
                                    custom={i}
                                    className="mb-3 grid grid-cols-2 items-end gap-2 border-b border-border/40 pb-3 md:grid-cols-5"
                                >
                                    <div>
                                        <Label>Día</Label>
                                        <Select
                                            ...
                                            onValueChange={(v) =>
                                                setData(
                                                    'horarios',
                                                    data.horarios.map((v2, idx) => (idx === i ? { ...v2, dia_en_texto: v } : v2)),
                                                )
                                            }
                                        >
                                            <SelectTrigger>
                                    ...
                                    <div>
                                        ...
                                        <Input
                                            ...
                                            onChange={(e) =>
                                                setData(
                                                    'horarios',
                                                    data.horarios.map((v, idx) => (idx === i ? { ...v, hora_inicio: e.target.value } : v)),
                                                )
                                            }
                                        />
                                    </div>
                                    <div>
                                        ...
                                        <Input
                                            ...
                                            onChange={(e) =>
                                                setData(
                                                    'horarios',
                                                    data.horarios.map((v, idx) => (idx === i ? { ...v, duracion_min: e.target.value } : v)),
                                                )
                                            }
                                        />
                                    </div>
                                    <div>
                                        ...
                                        <Select
                                            ...
                                            onValueChange={(v) =>
                                                setData(
                                                    'horarios',
                                                    data.horarios.map((v2, idx) => (idx === i ? { ...v2, sala: v } : v2)),
                                                )
                                            }
                                        >
                                            <SelectTrigger>
                                    ...
                                    <div>
                                        ...
                                        <Select
                                            ...
                                            onValueChange={(v) =>
                                                setData(
                                                    'horarios',
                                                    data.horarios.map((v2, idx) => (idx === i ? { ...v2, turno: v } : v2)),
                                                )
                                            }
                                        >
                                            <SelectTrigger>
                                    ...
                                    <div className="col-span-2 flex justify-end gap-2 md:col-span-5">
                                        <Button type="button" size="sm" variant="secondary" onClick={() => duplicateHorario(i)}>
                                            <PlusCircle className="mr-1 h-4 w-4" /> Duplicar
                                        </Button>

                                        <Button type="button" size="sm" variant="destructive" onClick={() => removeHorario(i)}>
                            ...
                            <Button type="button" onClick={addHorario} variant="secondary" className="mt-2">
                                <PlusCircle className="mr-1 h-4 w-4" /> Agregar horario
                            </Button>
                        </CardContent>
                    </Card>
