...
import { Asistencia, Curso, Inscripcion, pageProps } from '@/types';
...
export default function AlumnoShow() {
    ...
    const rutaVolver = origen === 'disponibles' ? route('alumno.cursos.index') : route('alumno.mis-cursos.index');
            ...
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 p-6">
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div className="flex justify-end">
                        {!miInscripcion && (
                            <Button onClick={() => router.post(route('cursos.preinscribir', curso.id))} className="bg-primary text-white">
                    ...
                    <CardContent className="grid grid-cols-1 gap-4 text-sm sm:grid-cols-2 lg:grid-cols-3">
                        ...
                        <div>
                            ...
                            <p className="font-medium">
                                {curso.profesores?.length ? curso.profesores.map((p) => `${p.nombre} ${p.apellido}`).join(' · ') : 'A asignar'}
                            </p>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <p className="font-medium">
                                {pct}% ({presentes}/{asistencias.length})
                            </p>
                        </div>
                    </CardContent>
                </Card>
                            ...
                            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                                {curso.horarios.map((h, i) => (
                                    <motion.div
                                        ...
                                        animate="visible"
                                        custom={i}
                                        className="flex flex-col gap-1 rounded-md border border-border/40 p-3 hover:bg-accent/10"
                                    >
